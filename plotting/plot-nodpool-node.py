...
def help():
    print('plot-nodpool-node.py -h          (help)')
    print('plot-nodpool-node.py -n 10,20    (nodes, starting from 1)')
    print('plot-nodpool-node.py -p 3        (number of plots in one row)')
    sys.exit()

prog=sys.argv[0]
try:
    opts, args = getopt.getopt(sys.argv[1:],"hn:p:",["help","nodes=","ncolumns="])
...
fig.savefig('nodpool-node.out.pdf', bbox_inches='tight', papertype='A0', dpi=600)
